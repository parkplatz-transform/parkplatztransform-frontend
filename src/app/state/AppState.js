...
class AppState {
	...
	async onSegmentCreated (segment) {
	  ...
	  async onSegmentEdited (updatedSegment) {
		...
		try {
		  this.setAlertDisplayed({severity: 'success', message: getString('segment_update_success')})
		  await updateSegment(updatedSegment)
		} catch (e) {
		  if (e instanceof PermissionsError) {
			this.setAlertDisplayed({severity: 'error', message: getString('permissions_failure')})
		  } else {
	  ...
	  async onSegmentSelect (id) {
		...
		if (id === this.selectedSegmentId) { return }
	...
	async onBoundsChanged(bounds) {
		...
		const latestModificationDate = this.getLatestModificationDate(loadedSegments)
		...
		try {
		  ...
		  if (geoJson.features && geoJson.features.length) {
			...
			console.log(e)
			this.setAlertDisplayed({severity: 'error', message: getString('segment_loaded_failure')})
			this.loadedBoundingBoxes = this.loadedBoundingBoxes.filter(bbox => bbox !== boundingBox)
		}	
	}
	
	checkIfBoundingBoxWasRequestedBefore (boundingBox) {
		return this.loadedBoundingBoxes.some(bbox => bboxContainsBBox(bbox, boundingBox))
	  }
	...
	async onSegmentDeleted (id) {
		try {
			this.setAlertDisplayed({severity: 'success', message: getString('segment_delete_success', 1)})
			this.segment = null
			delete this.allSegmentsById[id]
			await deleteSegment(id)
		} catch (e) {
			this.setAlertDisplayed({severity: 'error', message: getString('segment_delete_failure', 1)})
